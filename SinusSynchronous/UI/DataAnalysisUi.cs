...
public class DataAnalysisUi : WindowMediatorSubscriberBase
{
    ...
    private void DrawStoredData()
    {
        UiSharedService.DrawTree("What is this? (Explanation / Help)", () =>
        {
            ...
            UiSharedService.TextWrapped("Transient files are files that cannot be resolved to your character permanently. Sinus gathers these files in the background while you execute animations, VFX, sound effects, etc.");
            UiSharedService.TextWrapped("When sending your character data to others, Sinus will combine the files listed in \"All Jobs\" and the corresponding currently used job.");
            UiSharedService.TextWrapped("The purpose of this tab is primarily informational for you to see which files you are carrying with you. You can remove added game paths, however if you are using the animations etc. again, "
                + "Sinus will automatically attach these after using them. If you disable associated mods in Penumbra, the associated entries here will also be deleted automatically.");
        });
        ...
        using (ImRaii.Group())
        {
            ...
            using (ImRaii.Disabled(string.IsNullOrEmpty(_selectedJobEntry)))
            {
                ...
                UiSharedService.AttachToolTip("Hold CTRL to delete all game paths from the displayed list"
                    + UiSharedService.TooltipSeparator + "You usually do not need to do this. All animation and VFX data will be automatically handled through Sinus.");
                ImGuiHelpers.ScaledDummy(5);
    ...
    private void DrawRecording()
    {
        UiSharedService.DrawTree("What is this? (Explanation / Help)", () =>
        {
            UiSharedService.TextWrapped("This tab allows you to attempt to fix mods that do not sync correctly, especially those with modded models and animations." + Environment.NewLine + Environment.NewLine
                ...
                + "If it doesn't, Sinus is not able to catch the data or already has recorded the animation files (check 'Show previously added transient files' to see if not all is already present)." + Environment.NewLine + Environment.NewLine
        ...
        _uiSharedService.DrawHelpText("Use this only if you want to see what was previously already caught by Sinus");
        ImGuiHelpers.ScaledDummy(5);
    ...
    private void DrawAnalysis()
    {
        UiSharedService.DrawTree("What is this? (Explanation / Help)", () =>
        {
            UiSharedService.TextWrapped("This tab shows you all files and their sizes that are currently in use through your character and associated entities in Sinus");
        });
