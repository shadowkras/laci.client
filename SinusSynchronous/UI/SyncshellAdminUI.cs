...
public class SyncshellAdminUI : WindowMediatorSubscriberBase
{
            ...
            if (tabbar)
            {
                ...
                if (mgmtTab)
                {
                    ...
                    if (clearNode)
                    {
                        ...
                        UiSharedService.AttachToolTip($"This will start the prune process for this Syncshell of inactive Sinus users that have not logged in in the past {_pruneDays} days."
                        ...
                        if (_pruneTestTask != null)
                        {
                            ...
                            {
                                ImGui.AlignTextToFramePadding();
                                UiSharedService.TextWrapped($"Found {_pruneTestTask.Result} user(s) that have not logged into Sinus in the past {_pruneDays} days.");
    ...
    private static System.Numerics.Vector2 CalculateTableHeight(int rowCount)
    {
        float rowHeight = ImGui.GetTextLineHeightWithSpacing();
        float headerHeight = ImGui.GetTextLineHeightWithSpacing();
        float desiredHeight = headerHeight + (rowCount * rowHeight) + 10;
        var vectorHeight = new System.Numerics.Vector2(-1, desiredHeight);
        return vectorHeight;
    }
