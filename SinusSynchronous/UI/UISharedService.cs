...
public partial class UiSharedService : DisposableMediatorSubscriberBase
{
    ...
    {
        ColorTextWrapped("Note: The storage folder should be somewhere close to root (i.e. C:\\SinusStorage) in a new empty folder. DO NOT point this to your game folder. DO NOT point this to your Penumbra folder.", ImGuiColors.DalamudYellow);
        ...
        using (ImRaii.Disabled(_cacheMonitor.SinusWatcher != null))
        {
            ...
            {
                FileDialogManager.OpenFolderDialog("Pick Sinus Synchronous Storage Folder", (success, path) =>
                {
                    if (!success) return;
                    ...
                    {
                        _cacheDirectoryHasOtherFilesThanCache = true;
                        Logger.LogWarning("Found folders in {path} not belonging to Sinus: {dirs}", path, string.Join(", ", dirs));
                    }
        ...
        DrawHelpText("The storage is automatically governed by Sinus. It will clear itself automatically once it reaches the set capacity by removing the oldest unused files. You typically do not need to clear it yourself.");
    }
    ...
    public bool DrawOtherPluginState()
    {
        ...
        {
            ImGui.TextColored(ImGuiColors.DalamudRed, "You need to install both Penumbra and Glamourer and keep them up to date to use Sinus Synchronous.");
            return false;
        }
