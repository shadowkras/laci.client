...
public partial class IntroUi : WindowMediatorSubscriberBase
{
    ...
    protected override void DrawInternal()
    {
        ...
        {
            _uiShared.BigText("Welcome to Sinus Synchronous");
            ImGui.Separator();
            UiSharedService.TextWrapped("Sinus Synchronous is a plugin that will replicate your full current character state including all Penumbra mods to other paired Sinus Synchronous users. " +
                     ...
                     || !Directory.Exists(_configService.Current.CacheFolder)))
        {
            ...
            {
                UiSharedService.TextWrapped("To not unnecessary download files already present on your computer, Sinus Synchronous will have to scan your Penumbra mod directory. " +
                                     "Additionally, a local storage folder must be set where Sinus Synchronous will download other character files to. " +
                                     "Once the storage folder is set and the scan complete, this page will automatically forward to registration at a service.");
                UiSharedService.TextWrapped("Note: The initial scan, depending on the amount of mods you have, might take a while. Please wait until it is completed.");
                UiSharedService.ColorTextWrapped("Warning: once past this step you should not delete the FileCache.csv of Sinus Synchronous in the Plugin Configurations folder of Dalamud. " +
            ...
            if (!_dalamudUtilService.IsWine)
            {
                ...
                UiSharedService.ColorTextWrapped("The File Compactor can save a tremendeous amount of space on the hard disk for downloads through Sinus. It will incur a minor CPU penalty on download but can speed up " +
        ...
        else if (!_uiShared.ApiController.ServerAlive)
        {
            ...
            ImGui.Separator();
            UiSharedService.TextWrapped("To be able to use Sinus Synchronous you will have to register an account.");
            UiSharedService.TextWrapped("For the official Sinus Synchronous Servers the account creation will be handled on the official Sinus Synchronous Discord. Due to security risks for the server, there is no way to handle this sensibly otherwise.");
            UiSharedService.TextWrapped("If you want to register at the main server \"" + WebAPI.ApiController.MainServer + "\" join the Discord and follow the instructions as described in #sinus-service.");

            if (ImGui.Button("Join the Sinus Synchronous Discord"))
            {
